...
import { SharingLink, SharingLinksApi } from "shared-api-client";
...
export default function useAddSharingLink() {
  ...
  return useMutation(
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries(wellKnownQueries.sharingLinksGetAll);
        queryClient.invalidateQueries(wellKnownQueries.sharingLinksGetLatest);
      },
      onError: (error) => {
        console.log(error);
      },
