...
import { FilterSetting, FilterSettingsApi } from "shared-api-client";
...
export default function useSaveFilterSettings() {
  ...
  return useMutation(
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries(wellKnownQueries.filterSettingLatest);
        queryClient.invalidateQueries(wellKnownQueries.filterSettingsAll);
      },
    }
  );
}
